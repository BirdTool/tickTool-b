...
createResponder({
    ...
    async run(interaction, { stap }) {
        if (stap === "embed") {
            const Embed = await db.guilds.get<Embed[]>("embeds") || [];
            const tickets = await db.guilds.get<TicketDetails[]>("tickets") || [];
            const choice = interaction.values[0]
    
            if (!Embed.some((embed: Embed) => embed.name === choice)) { // verifica se o embed existe
            ...
            if (!embedChoiceName || !Embed.some((embed: Embed) => embed.name === embedChoiceName)) {
                return interaction.reply({
            ...
            const selectedEmbed = Embed.find(embed => embed.name === embedChoiceName) as Embed;
