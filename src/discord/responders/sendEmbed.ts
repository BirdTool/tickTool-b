...
import { ButtonBuilder, ButtonStyle, ChannelSelectMenuBuilder, ChannelType, EmbedBuilder, StringSelectMenuBuilder, TextChannel } from "discord.js";
...
createResponder({
    ...
    async run(interaction, { stap }) {
        const embedJson = await db.guilds.get<Embed[]>("embeds") || []
        if (interaction.isButton()) {
            switch(stap) {
                case "optionsEmbed": {
                    ...
                    const options = (getEmbeds).map(name => ({ // transforma em um valor válido pro selectMenu
                        label: name.name,
                        value: name.name,
                        emoji: "📜"
                    }));
                ...
                case "sendWithTicket": {
                    ...
                    const hasTicket: boolean = embedChoice ? Array.isArray(embedChoice.tickets) && embedChoice.tickets.length > 0 : false;
                    if (!hasTicket) {
            ...
            switch(stap) {
                case "choiceEmbed": {
                    ...
                    const hasTicket: boolean = embedChoice ? Array.isArray(embedChoice.tickets) && embedChoice.tickets.length > 0 : false;
                    const hasTicketInvert: boolean = hasTicket === true ? false : true; // inverter o booleano de hasTicket
            ...
            switch(stap) {
                ...
                case "channelTicket": {
                    ...
                    // Envia o embed no canal escolhido
                    await (channel as TextChannel).send({ embeds: [embed], components: [componentsMenu] });
