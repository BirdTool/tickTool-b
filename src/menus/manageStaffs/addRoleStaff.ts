import { ButtonBuilder, ButtonStyle, RoleSelectMenuBuilder, SelectMenuDefaultValueType, StringSelectMenuBuilder, type InteractionReplyOptions } from "discord.js";
import { createEmbed, createRow } from "@magicyan/discord";
import { settings } from "#settings";
import { createResponder, ResponderType } from "#base";
import { db } from "#database";

export function addRoleStaffMenu<R>(position?: string, role?: string): R {
...
createResponder({
    ...
    async run(interaction) {
        if (interaction.isStringSelectMenu()) {
            ...
            if (!position) return interaction.reply({content: "Selecione uma posição!", flags})
            ...
            if (roles.superAdmins.includes(role) || roles.admins.includes(role) || roles.moderators.includes(role)) return interaction.reply({content: "Este cargo já está na lista de staffs", flags})
            if (!role) return interaction.reply({content: "Selecione um cargo!", flags})
            return interaction.update(addRoleStaffMenu(position, role));
        } else if (interaction.isButton()) {
            const position = interaction.message.embeds[0].fields[0].value;
            const role = interaction.message.embeds[0].fields[1].value;
            if (!position || !role) return interaction.reply({content: "Selecione uma posição e um cargo!", flags})
            const positionFormated = position === "superAdmin" ? "superAdminRoles" : position === "admin" ? "adminRoles" : "modRoles";
            const roles = await db.guilds.get<string[]>(positionFormated) || [];
            roles.push(role);
