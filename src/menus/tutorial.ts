...
export function tutorialMenu<R>(step: number = 0): R {
...
createResponder({
    ...
    async run(interaction, { step }) {
        ...
        if (stepNumber === -1) return interaction.reply("Não é possivel voltar a um passo que não existe");
        if (stepNumber > steps.length) return interaction.reply("Não é possivel avançar pra um passo que não existe")
